  ...
  const onSubmit = async (data: RegisterInput) => {
    try {
      ...
      if (Register.fulfilled.match(resultAction)) {
        ...
        console.error("Register failed:", resultAction.error);
        toast.error(`Registeration failed: ${resultAction.error}`);
      }
    } catch (error) {
  ...
  return (
    <Form {...form}>
      <form className="space-y-24" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <Button className="flex justify-center items-center w-full bg-primary text-background py-2 mt-5 whitespace-nowrap font-medium px-4 hover:bg-primary/90 h-10 rounded-md text-sm" onClick={() => navigate('/login')}  >Login</Button>
      </div>
    </form>
    </Form >
  );
